...
class DaGMM(nn.Module):
    """Residual Block."""
    def __init__(self, n_gmm = 2, latent_dim=3,input_dim=118):
...
class DaGMMVAE(nn.Module):
    """Residual Block."""
    def __init__(self, n_gmm = 2, latent_dim=3,input_dim=118):
        super(DaGMMVAE, self).__init__()
        ...
        layers += [nn.Tanh()]        
        self.encoder = nn.Sequential(*layers)
        self.layer_mean = nn.Linear(10,1)
        self.layer_std = nn.Linear(10,1)
    ...
    def encode(self, x):
        h = self.encoder(x)
        return self.layer_mean(h), self.layer_std(h)
    ...
    def forward(self, x):
        ...
        enc = self.reparameterize(mean, std)

        dec = torch.sigmoid(self.decoder(enc))
        ...
        return enc, dec, z, gamma,mean,std
    ...
    def loss_function(self, x, x_hat, z, gamma, lambda_energy, lambda_cov_diag,mean,std):
        ...
        sample_energy, cov_diag = self.compute_energy(z, phi, mu, cov)
        kl_div = - 0.5 * torch.sum(1 + std - mean.pow(2) - std.exp())

        loss = recon_error + lambda_energy * sample_energy + lambda_cov_diag * cov_diag

        return loss, sample_energy, recon_error, cov_diag,kl_div
